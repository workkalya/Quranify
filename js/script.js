...
async function displayAlbums() {
    ...
    try {
        ...
        div.innerHTML = text;
        const anchors = div.querySelectorAll("a.icon-directory");

        // Log all anchors for debugging
        console.log("All anchors:", anchors);

        // Filter out folders from the directory listing
        const folders = Array.from(anchors)
            .map((anchor) => anchor.querySelector("span.name").textContent.trim()); // Get folder names

        console.log("Detected folders:", folders);

        // If no folders are detected, log a warning
        if (folders.length === 0) {
            console.warn("No folders detected. Check the HTML structure of the directory listing.");
            return;
        }
        ...
        const metadataPromises = folders.map(async (folder) => {
            try {
                const metadataResponse = await fetch(`http://127.0.0.1:5500/songs/${folder}/info.json`);
                ...
                return null; // Skip folders with errors
            }
        });

        // Wait for all metadata requests to complete
        const foldersWithMetadata = (await Promise.all(metadataPromises)).filter(Boolean);

        // Render cards in the HTML
